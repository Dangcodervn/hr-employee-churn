...
FROM [hr-employee-churn].[dbo].[employees_raw];
...
WITH tmp AS (
    SELECT
        ...
        CASE
            ...
            WHEN tenure <= 3 THEN '1-3 nam'
            WHEN tenure <= 5 THEN '3-5 nam'
            ELSE '5+ nam'
        END AS tenure_group,
        churn
    FROM [hr-employee-churn].[dbo].[employees_raw]
...
SELECT
    ...
    MIN(salary) AS min_salary,
    MAX(salary) AS max_salary,
    CAST(AVG(performance_rating) AS DECIMAL(3,2)) AS avg_performance_rating
FROM tmp
...
WITH tmp AS (
    SELECT
        ...
        CASE
            ...
            WHEN overtime_hours <= 10    THEN '1-10'
            WHEN overtime_hours <= 20    THEN '11-20'
            ELSE 'over 20'
...
WITH dept_abs AS (
    SELECT
        department,
        COUNT(*) AS total_employees,
        CAST(AVG(CAST(absenteeism AS DECIMAL(10,2))) AS DECIMAL(10,2)) AS avg_absenteeism,
        SUM(absenteeism) AS total_absenteeism
    FROM [hr-employee-churn].[dbo].[employees_raw]
    GROUP BY department
)
SELECT TOP 10
...
FROM dept_abs
ORDER BY total_absenteeism DESC, department;
...
WITH salary_ranked AS (
    SELECT
        ...
        salary,
        churn,
        CAST(AVG(salary) OVER (PARTITION BY job_role) AS DECIMAL(12,2)) AS role_avg_salary,
...
ORDER BY job_role, salary_percentile, salary;
...
WITH salary_ranked AS (
    SELECT
        ...
        CAST(
            ROUND(
                (salary - AVG(salary) OVER (PARTITION BY job_role))
                / NULLIF(AVG(salary) OVER (PARTITION BY job_role), 0) * 100.0,
                2
            ) AS DECIMAL(8,2)
        ) AS salary_gap_pct
    FROM [hr-employee-churn].[dbo].[employees_raw]
